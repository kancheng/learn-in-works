...
// processCode
void processCode (char *varNameTable[], int code[], int opd1[], int opd2[], int compopr[], int gotoLine[], int target[]) 
{
	char first[NAMELENGTH];
	char opd[NAMELENGTH];
	int lineNum = 2; // 1 is for var
	char *firstTable[] = { "STOP", "IF", "GOTO", "PRINT", NULL};
	int firstTableSize = sizeof(firstTable) / sizeof(char *);
...
// runCode
void runCode (char varName[][NAMELENGTH], int varValue[], int code[], int opd1[], int opd2[], int compopr[], int gotoLine[], int target[])
{
	...
	while (code[line] != STOP)
	{
		switch (code[line])
		{
			...
			case GOTO:
				...
				break;
			case ASSIGN:
				varValue[target[line]] = arithmetic(varValue[opd1[line]], varValue[opd2[line]], compopr[line]);
				line++;
				break;
			case PRINT:
