...
ListNode *insertLinkedList (ListNode *head, int data) {
	...
	head->next = insertLinkedList (head->next, data);
	return (head);
}
// delete
ListNode *deleteLinkedList (ListNode *head, int data) {
	ListNode *temp;
	if (head == NULL)
		return NULL;
	if (data == head->data) {
	...
	head->next = deleteLinkedList(head->next, data);
	return (head);
}
// freeLinkedList
...
int main(void)
{
	...
	ListNode *head = NULL;
	for (int i = 0; i < INSKEY; i++)
		head = insertLinkedList(head, insertKeys[i]);
	printLinkedList(head);
	for (int i = 0; i < DELKEY; i++)
		head = deleteLinkedList(head, deleteKeys[i]);
	printLinkedList(head);
	freeLinkedList(head);
	return 0;
}