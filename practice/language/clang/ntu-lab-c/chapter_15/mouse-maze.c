...
bool goMaze (int maze[MAZESIZE][MAZESIZE],
			int step, int currentRow, int currentColumn, int endRow, int endColumn,
			int n, int m) {
	int drow[4] = { 1, 0, -1, 0};
	...
	for (int d = 0; d < 4; d++) {
		...
		if (inMaze(nextRow, nextColumn, n, m) && maze[nextRow][nextColumn] == 0) {
			...
			if (goMaze(maze, step + 1, nextRow, nextColumn, endRow, endColumn, n, m))
				return true;
...
int main (void) {
	...
	goMaze(maze, 2, startRow, startColumn, endRow, endColumn, n, m);
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < m; j++)
			printf("%2d ", maze[i][j]);
		printf("\n");
